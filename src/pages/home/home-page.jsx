    ...
    <main className="home">
      <section className="hero">
        <header className="header">
          <a className="logo" href="#top"><span>ART</span><span>KANTE</span><small>БЮРО КАМИНОВ</small></a>
          <div className="navigation">
            <nav className="top-nav"><a href="#">Философия</a><a href="#">Проектирование</a><a href="#">Портфолио</a><a href="#">Вдохновение</a><a href="#">Контакты</a><a href="#">Избранное</a></nav>
            <nav className="category-nav"><a href="#cards">ЗАГОРОДНЫЙ ДОМ⌄</a><a href="#cards">ГОРОДСКАЯ КВАРТИРА⌄</a><a href="#cards">ОБЩЕСТВЕННЫЕ ПРОСТРАНСТВА⌄</a><a href="#cards">ОБЛИЦОВКА⌄</a></nav>
          </div>
          <button className="menu" type="button">☰</button>
        </header>
        <div className="hero-text" id="top"><p className="title">ПЕЧИ КАМИНЫ БАРБЕКЮ <span>ПОД КЛЮЧ</span></p><h1>проектирование архитектура инженерия монтаж</h1><p className="slogan">С НАМИ ЛЕГКО ВНЕДРЯЮТСЯ ПРОЕКТЫ</p><div className="arrows"><button>←</button><button>→</button></div></div>
      <div className="fire-glow" aria-hidden="true" />
      </section>
      <section className="cards-section" id="cards"><p>НАПРАВЛЕНИЯ</p><h2>Пространства,<br />которые становятся вашими</h2><div className="cards">{cards.map(([name, image], index) => <a className={`card card-${index + 1}`} href="#" key={name}><img src={image} alt="" /><span /><strong>{name}</strong><em>↗</em></a>)}</div></section>
